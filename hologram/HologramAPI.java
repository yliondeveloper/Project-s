...
public abstract class HologramAPI {
	...
	protected static Collection<? extends Player> checkReceiverWorld(Hologram hologram,
	...
	protected static int getVersion(Player p) {
		try {
			...
			try {
				version = AccessUtil
						.setAccessible(network.getClass().getDeclaredMethod("getVersion",
								new Class[] { NMUClass.io_netty_channel_Channel }))
						.invoke(network, new Object[] { channel });
			} catch (Exception e) {
				return 182;
			}
	...
	public static void enablePacketListener() {
		new PacketListener(Main.instance);
	}

	public static void disablePacketListener() {
	...
	protected static List<Hologram> holograms = new ArrayList<Hologram>();
